...
Across the case study, the **spatial aggregation method** — not resolution or the choice between k-anonymity and l-diversity — is the primary driver of utility loss. Uniform grid methods (S2, H3, Triangle) preserve utility best and remain robust as privacy stringency increases, while non-uniform methods, particularly the Voronoi diagram and coarse administrative boundaries, incur substantially higher utility loss.
...
> \_Fill in to match your actual layout, e.g.:\_
...
> \_Fill in your actual setup steps, e.g.:\_
...
> \_Fill in how to run the framework/pipeline and the interface, e.g.:\_

```bash
# run the evaluation framework on a dataset
python framework/run\_evaluation.py --config config.yaml
...
> Sirinapaphan, S. (2026). \*The Privacy-Utility Trade-off in Spatial Aggregation: An Evaluation Framework for Origin-Destination Flow Visualization\* \[Master's thesis]. \[Your Institution].

## License

> \_Add a license (e.g., MIT) if you intend this to be reused by others.\_
